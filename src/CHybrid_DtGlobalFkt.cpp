...
void CHybrid::sum_up_block_timing(void)
{
	...
	//! Implicit: only called when do_timing is true
...
// 	synchronize_allProcesses_MPI();
	...
	if constexpr (DELETE_PARTICLE_IF_V_EXCEEDS)
...
bool CHybrid::LF_at_cycle(INT32 level, INT32 CYCLE)
{
	...
	if constexpr (nonadiabatic_gradPE_TERM && nonadiabatic_gradPE_TERM_advance_Pe_with_B)
...
void CHybrid::advanceB_LF_globalDT(void)
{
	...
	else if constexpr (nonadiabatic_gradPE_TERM && !nonadiabatic_gradPE_TERM_advance_Pe_with_B)
	...
	{
		if constexpr (nonadiabatic_gradPE_TERM_smooth_PE)
		{
...
void CHybrid::show_PE(INT32 species)
{
	...
	if constexpr (nonadiabatic_gradPE_TERM)
		return; //  nothing to do because PE is already computed
	else
	{
		for(INT32 level=0; level<=MAX_LEVEL; level++)
		{
