...
void CHybrid::mark_particle(void)
{
	...
	if(serialize_marking_of_Particles && mpi_myRank)
	  ...
	  MPI_Recv(particle_counter_of_species,
		   num_entries,
		   MPI_INT64,
		   mpi_myRank-1,
		   tag,
...
// 			temp_Block->write_particle_number();
	...
	{	
		...
		MPI_Send(particle_counter_of_species, num_entries, MPI_INT64, mpi_myRank+1, tag, mpi_myComm);
...
// 		silo_2DWrite_allCS(id_PISpecies1 +0);
	...
	if constexpr (nonadiabatic_gradPE_TERM)
	{
...
//!	a given Cross Section Coordinate specified in parameters.h.
...
void CHybrid::write_Field3D(INT32 FType, INT32 TL, bool write_state)
{
	...
	if(serialize_writing_of_StateFile && mpi_myRank)
	  //mpi_myComm.Recv(&is_finished, length_in_byte, MPI_INT32, mpi_myRank-1, tag );
	  MPI_Recv(&is_finished, length_in_byte, MPI_INT32, mpi_myRank-1, tag, mpi_myComm, MPI_STATUSES_IGNORE);
	...
	{	
		...
		usleep(TIME_TO_WAIT);
		//mpi_myComm.Send(&is_finished, length_in_byte, MPI_INT32, mpi_myRank+1, tag);
                MPI_Send(&is_finished, length_in_byte, MPI_INT32, mpi_myRank+1, tag, mpi_myComm);
