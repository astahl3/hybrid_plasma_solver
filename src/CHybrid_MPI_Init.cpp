...
void CHybrid::init_MPI(int argc, char *argv[])
{
	...
	//mpi_num_processes = mpi_myComm->Get_size();
        MPI_Comm_rank(mpi_myComm, &mpi_myRank);
        MPI_Comm_size(mpi_myComm, &mpi_num_processes);
...
// 	log_file << " synchronizing ... " << endl;
	//mpi_myComm.Barrier(); // ND 05/04/18
        MPI_Barrier(mpi_myComm);
...
void CHybrid::check_for_NaN_MPI(bool is_NaN_this_process)
{
	...
	//MPI_INT,
	//MPI_SUM);
        MPI_Allreduce(NaN_in_process_in,
		      NaN_in_process_out,
		      mpi_num_processes,
		      MPI_INT,
		      MPI_SUM, mpi_myComm);
