...
void CHybrid::inject_obstacle_ions(void)
{
	...
	if constexpr (use_neutral_species_as_field)
	{
		if constexpr (ion_prod_profile_from_file)
			insert_ions_from_ionprod_file();
		else
...
void CHybrid::insert_ions_from_ionprod_file()
{
	clock_t start,finish;
	double time;
	if constexpr (do_timing)
...
void CHybrid::insert_ions_from_neutral_profile(void)
{
	...
	{	
		...
		insert_every_x_TL= int(insert_every_x_TL*num_newlyionised_particles/total_particles_to_insert + 1);	
		log_file << "insert every " << insert_every_x_TL << " TL" << endl;
		log_file << "num newlyionized particles:  " << num_newlyionised_particles << endl;
		log_file << "total particles to insert:   " << total_particles_to_insert << endl << endl;
		//insert_every_x_TL = 1;
	}
...
// 		}
	...
	{	
		...
		//		MPI_D_REAL,
		//		MPI_MAX);
                MPI_Allreduce(max_ChemicalReaction_Probability_local,
			      max_ChemicalReaction_Probability_global,
			     (num_Particle_Species)*(num_Particle_Species+1)+1,
                              MPI_D_REAL, MPI_MAX, mpi_myComm);
