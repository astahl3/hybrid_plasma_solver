...
void CHybrid::set_Field_Comps(void)
{
	...
	if constexpr (nonadiabatic_gradPE_TERM || use_neutral_species_as_field)
	{
	...
	if constexpr (use_ion_production_as_field)
	{
	...
	if constexpr (use_dust_species_as_field)
	{
...
void CHybrid::do_consistency_check_init_derived_variables(void)
{
	...
	if(7 +2*num_Charged_Species +(MAX_LEVEL+1) > INFO_ARRAY_SIZE)
	{
		...
		log_file << " see parameters.h for further details. " <<endl;
		log_file << " Exiting ... " << endl;
		exit(1);
	...
	if( id_average_Field1 +(num_average_fields-1) >= NUM_FIELDS)
	{
		...
		log_file << "   id_average_Field1"		<< endl
		     ...
		     << " increase NUM_FIELDS i parameters.h to at least "
	...
	if constexpr (USE_CFBG_BFIELD)
	{
		if( !vec_len(Magnetic_Moment))
		{
			...
			log_file << "   file 'parameters.h'. " << endl;
			log_file << "   EXITING ... " << endl;
			exit(1);
		}
	}
	...
	if(vec_len(Magnetic_Moment))
	  {
	    ...
	    log_file << "   Standoff distance from Callisto = " << pow(mu_0 * pow(vec_len(MM),2)/(8*pi*(ram + thermal + mag)),1./6.)/2410000 << " Rc" << endl << endl;
	    log_file << "-------------------------------------------------------------------" << endl << endl;
	  }
...
//! Below required DUST_FIELD and DUST_PARTICLE to be declared/defined somewhere
// #if DUST_FIELD

// 	if(num_externRhoVelocityFields<2)
// 	{
// 		log_file << " DUST_FIELD is set in parameters.h but num_externRhoVelocityFields is < 2 ... " <<endl;
// 		log_file << " EXITING ... " << endl;
// 		finalize_MPI();
// 		exit(1);
// 	}
...
// 	if(num_externRhoVelocityFields<2)
// 	{
// 		log_file << " DUST_PARTICLE is set in parameters.h but num_externRhoVelocityFields is < 2 ... " <<endl;
// 		log_file << " EXITING ... " << endl;
// 		finalize_MPI();
// 		exit(1);
// 	}
...
// 	#if DUST_PARTICLE
// 		log_file << " DUST_FIELD and DUST_PARTICLE are set in parameters.h ... " <<endl;
// 		log_file << " EXITING ... " << endl;
// 		finalize_MPI();
// 		exit(1);
// 	#endif
...
// 	{
// 		log_file << " num_Charged_Species = num_Particle_Species but DUST_FIELD is set in parameters.h ... " <<endl;
// 		log_file << " EXITING ... " << endl;
// 		finalize_MPI();
// 		exit(1);
// 	}	
...
// 	{
// 		log_file << " num_Inflow_Species = num_Particle_Species but DUST_PARTICLE is set in parameters.h ... " <<endl;
// 		log_file << " EXITING ... " << endl;
// 		finalize_MPI();
// 		exit(1);
// 	}
// #endif
	...
	if constexpr (!TRACK_PARTICLE)
	{
		for(INT32 species=0; species<num_Charged_Species; species++)
		{
			...
			{
				log_file << "ERROR: "  << endl;
				log_file << " but TRACK_PARTICLE is not defined in parameters.h" << endl;
	...
	/*
		if(num_mark_total_particle<=0)
		{
			...
			log_file << "  num_mark_total_particle = "<< num_mark_total_particle << endl
				<< " but TRACK_PARTICLE is defined in parameters.h" << endl;
	...
	{
		log_file << "Error:" << endl
			<< "num_Neutral_Species (parameters.h):     " << num_Neutral_Species << endl
			<< "NUM_NEUTRAL_SPECIES (parameters.h): " << NUM_NEUTRAL_SPECIES << endl
	...
	{
		log_file << "Error:" << endl
			<< "num_Particle_Species (parameters.h):     " << num_Particle_Species << endl
			<< "NUM_PARTICLE_SPECIES (parameters.h): " << NUM_PARTICLE_SPECIES << endl
	...
	log_file <<  " Ion Beta = "<<Ion_Betas[0]<<endl<<endl;

	log_file <<  " Thermal Gyroradius = "<<sqrt(3*SI_m0*Ion_Betas[0])/(R_Moon*SI_x0*e*sqrt(2*mu_0*SI_n0))<<" R_Moon "<<endl<<endl;
...
#if (NONADIABATIC_GRADPE_TERM) && (nonadiabatic_gradPE_TERM_advance_Pe_with_B)
...
#if defined(startTL_PROTOCOL_OBSTACLE_PARTICLE)
...
// 	double loop_time_phys, run_time_phys;
	...
	F_REAL TL_cell_crossing, min_sw_crosses_cell, min_sw_crosses_Box, BoxCrossings_since_start;
	F_REAL min_Simu_finished;

	char crossing_value[10];

	if constexpr (do_timing || ESTIMATE_FASTEST_PARTICLE)
	{
		TL_processed = (TL -TL_at_last_restore_state);
	...
	// 	log_file << "   -> Avrg. s/TL (phys. time): " << run_time_phys / TL_processed << "s." << endl;
		...
		TL_cell_crossing = delta_of_L[MAX_LEVEL][0]/( vec_len(V_sw) * dt);

		min_sw_crosses_cell = (TL_cell_crossing*loop_time_mpi)/60.;
#if (use_vectorclass)
		min_sw_crosses_Box  = RB_X*(BlkNds_X-2)*min_sw_crosses_cell *pow(double(2),double(MAX_LEVEL));
		BoxCrossings_since_start = (1.*TL) / (pow(2.,double(MAX_LEVEL))*RB_X*(BlkNds_X-2)*TL_cell_crossing);
#else
		min_sw_crosses_Box  = RB_X*(BlkNds_X-2)*min_sw_crosses_cell *pow(2,MAX_LEVEL);
		BoxCrossings_since_start = (1.*TL) / (pow(2,MAX_LEVEL)*RB_X*(BlkNds_X-2)*TL_cell_crossing);
#endif

		sprintf(crossing_value,"%2.3f",BoxCrossings_since_start);

		min_Simu_finished = (loop_time_comp*(TL_MAX-TL))/60.;
	}

	if constexpr (ESTIMATE_FASTEST_PARTICLE)
	{
        ...
        if(tofast)
        {
                ...
                log_file <<" !!!                   (or)                        !!!" << endl;
                log_file <<" !!!     (limit maximal v_part in parameters.h)    !!!" << endl;
                log_file <<" !!!-----------------------------------------------!!!" << endl;
        }
	}
